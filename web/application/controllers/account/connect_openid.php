...
class Connect_openid extends CI_Controller {
	...
	function index()
	{
		...
		if ($this->input->get('janrain_nonce'))
		{
			...
			$response = $consumer->complete(site_url('index.php/account/connect_openid'));
		...
		$this->form_validation->set_rules(array(
			array(
				'field' => 'connect_openid_url',
				'label' => 'lang:connect_openid_url',
				'rules' => 'trim|required'
			)
		));
		...
		if ($this->form_validation->run())
		{
			...
			else
			{
				...
				header("Location: ".$auth_request->redirectURL(base_url(), site_url('index.php/account/connect_openid')));
			}
		}
