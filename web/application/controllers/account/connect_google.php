...
class Connect_google extends CI_Controller {
	...
	function index()
	{
		...
		//LIGHTTPD FIX 
		$fixLoc = $auth_request->redirectURL(base_url(), site_url('account/connect_google'));
		$fixLoc = str_replace('https%3A%2F%2Fwww.android-feedback.com%2Faccount%2Fconnect_google%3', 'https%3A%2F%2Fwww.android-feedback.com%2Findex.php%2Faccount%2Fconnect_google%3', $fixLoc);

		//
		// Redirect to authorizate URL
		//header("Location: ".$auth_request->redirectURL(base_url(), base_url()."index.php/account/connect_google"));
