...
class Account_profile extends CI_Controller {
	...
	function index($action = NULL)
	{
		...
		if ($action == 'delete')
		{
			unlink(FCPATH.'assets/user/profile/'.$data['account_details']->picture); // delete previous picture
		...
		if ($this->form_validation->run())
		{
			...
			if (isset($_FILES['account_picture_upload']) && $_FILES['account_picture_upload']['error'] != 4)
			{
				...
				$this->load->library('upload', array(
					'overwrite' => TRUE,
					'upload_path' => FCPATH.'assets/user/profile',
				...
				else
				{
					...
					$this->image_lib->initialize(array(
						'image_library' => 'gd2',
						'source_image' => FCPATH.'assets/user/profile/'.$picture['file_name'],
						'new_image' => FCPATH.'assets/user/profile/pic_'.md5($data['account']->id).$picture['file_ext'],
					...
					unlink(FCPATH.'assets/user/profile/'.$picture['file_name']);
				}
			}
